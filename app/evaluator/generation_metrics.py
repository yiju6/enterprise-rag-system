...
async def compute_generation_metrics(
    ...
    answer_correctness_scorer = AnswerCorrectness(llm=llm)

    context_recall, faithfulness, answer_relevancy, answer_correctness = await asyncio.gather(
        context_recall_scorer.ascore(user_input=question,
        retrieved_contexts=retrieved_contexts,
        reference=ground_truth),  
        faithfulness_scorer.ascore(user_input=question,
        generated_answer=answer,
        retrieved_contexts=retrieved_contexts), 
        answer_relevancy_scorer.ascore(user_input=question,
        generated_answer=answer,
        retrieved_contexts=retrieved_contexts), 
        answer_correctness_scorer.ascore(user_input=question,
        generated_answer=answer,
        reference=ground_truth)
    )
